...
Scheduled log機能では、Terraformがscheduled-log用ECR repository、CloudWatch Log Group、EventBridge Scheduler、Scheduler用IAM role、Scheduler target検証用のbootstrap task definition revisionを管理する。Backend側のdeploy workflowは `scheduled_log_ecr_repository_url` にimageをpushし、同じ `scheduled_log_task_definition_family` に本番用task definition revisionを登録する。`scheduled_log_scheduler_state` のdefaultは `ENABLED` なので、通常の `terraform apply` でSchedulerは有効化される。GitHub Actions backend deploy roleにはScheduler更新権限を付与しない。
