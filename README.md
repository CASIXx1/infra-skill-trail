...
BackendのECS service/task definitionはecspresso側で管理する。Terraform側ではECS cluster、IAM role、network、security group、ALB、CloudWatch LogsなどのAWS基盤を管理する。ECR repositoryは`environments/shared`で管理し、`environments/dev`から参照する。
